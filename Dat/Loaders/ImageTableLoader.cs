...
{
	public static ImageTable CreateImageTable(IImageTableNameProvider imageNamer, ObjectType objectType, List<GraphicsElement> imageList)
	{
		NameImages(imageNamer, imageList);
	...
	private static void NameImages(IImageTableNameProvider imageNamer, List<GraphicsElement> imageList)
	{
		for (var i = 0; i < imageList.Count; ++i)
		{
			imageList[i].Name = imageNamer.TryGetImageName(i, out var name)
				? name
				: DefaultImageTableNameProvider.GetImageName(i);
		}
	}

	private static ImageTable CreateImageTable(AirportObject airportObject, List<GraphicsElement> imageList)
	{
		var imageTable = new ImageTable();

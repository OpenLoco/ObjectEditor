// Library of one-off database/object helper scripts. Each public method is exposed as a
// runnable script in the Helpers tab. Behaviour is preserved from the legacy DataSanitiser
// console app; only hardcoded paths have been replaced with values from ToolsSettings, and
// Console.WriteLine output is captured by the UI via a redirected TextWriter.
...
public static class DatabaseHelperScripts
{
	...
	public static Task QueryBuildingVar_AC(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		...
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		foreach (var obj in index.Objects.Where(x => x.ObjectType == ObjectType.Building))
		{
			...
			{
				log($"{obj.FileName} - {ex.Message}");
			}
		}

		log(results.Count.ToString());
		const string csvHeader = "DatName, Var_AC";
		var lines = results.OrderByDescending(x => x.var_ac).Select(x => string.Join(',', x.Obj.DisplayName, x.var_ac));
		var outFile = Path.Combine(settings.JsonDirectory, "building_var_ac.csv");
		File.WriteAllLines(outFile, [csvHeader, .. lines]);
		log($"Wrote {outFile}");
	});

	public static Task QueryBuildingProducedQuantity(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		var lines = results.OrderBy(x => x.Item2.ProducedQuantity).Select(x => string.Join(',', x.Obj.DisplayName, x.Item2.ProducedName, x.Item2.ProducedQuantity));
		var outFile = Path.Combine(settings.JsonDirectory, "buildingProducedQuantities.csv");
		File.WriteAllLines(outFile, [csvHeader, .. lines]);
		log($"Wrote {outFile}");
	});

	public static Task QueryHeadquarters(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		log(results.Count.ToString());
		foreach (var line in results.OrderBy(x => x.Obj.DisplayName).Select(x => string.Join(',', x.Obj.DisplayName, x.ObjectSource)))
		{
			log(line);
		}
	});

	public static Task QueryCostIndex(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		{
			log($"Type: {type} implements ILocoStruct and has a CostIndex property.");
		}
		...
		{
			log($"{result.Obj.DisplayName} - {result.ObjectSource} - {result.ObjectType} - CostIndex={result.CostIndex}");
		}
	});

	public static Task QueryTrackStationOneSidedTrack(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		foreach (var obj in index.Objects.Where(x => x.ObjectType == ObjectType.TrackStation))
		{
			try
			{
				...
				if (o.LocoObject != null)
				{
					...
					{
						log($"{header.Name} didn't have it");
					}
				}
			}
			catch (Exception ex)
		...
		{
			log($"{result.Obj.DisplayName} - Checksum: 0x{result.Obj.DatChecksum:X} - Source: {result.ObjectSource} - Flags: {string.Join('|', result.Flags)}");
		}
	});

	public static Task QueryIndustryHasShadows(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		var lines = results.OrderBy(x => x.Obj.DisplayName).Select(x => string.Join(',', x.Obj.DisplayName, x.ObjectSource));
		var outFile = Path.Combine(settings.JsonDirectory, "industryHasShadows.csv");
		File.WriteAllLines(outFile, [csvHeader, .. lines]);
		log($"Wrote {outFile}");
	});

	public static Task QueryVehicleBodyUnkSprites(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		const string csvHeader = "DatName, ObjectSource";
		var lines = results.OrderBy(x => x.Obj.DisplayName).Select(x => string.Join(',', x.Obj.DisplayName, x.ObjectSource));
		var outFile = Path.Combine(settings.JsonDirectory, "vehicleBodiesWithUnkSpritesFlag.csv");
		File.WriteAllLines(outFile, [csvHeader, .. lines]);
		log($"Wrote {outFile}");
	});

	public static Task QueryCargoCategories(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		var lines = results.OrderBy(x => x.Obj.DisplayName).Select(x => string.Join(',', x.Obj.DisplayName, (int)x.CargoCategory, x.LocalisedName, x.ObjectSource));
		var outFile = Path.Combine(settings.JsonDirectory, "cargoCategories.csv");
		File.WriteAllLines(outFile, [csvHeader, .. lines]);
		log($"Wrote {outFile}");
	});

	public static Task QueryCostIndices(ToolsSettings settings, Action<string> log) => Task.Run(() =>
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		var lines = results.OrderBy(x => x.Obj.DisplayName).Select(x => string.Join(',', x.Obj.DisplayName, x.Obj.ObjectType, x.CostIndex, x.RunCostIndex));
		var outFile = Path.Combine(settings.JsonDirectory, "costIndex.csv");
		File.WriteAllLines(outFile, [header, .. lines]);
		log($"Wrote {outFile}");
	});
	...
	public static async Task WritexxHash3(ToolsSettings settings, Action<string> log)
	{
		...
		foreach (var lookup in objects)
		{
			...
			{
				var filename = Path.Combine(dir, entry.FileName);
		...
		log($"Database path: {settings.DatabaseFile}");
	}
		...
		foreach (var obj in objects)
		{
			if (index.TryFind((obj.DatObjects.First().DatName, obj.DatObjects.First().DatChecksum), out var entry))
			{
				...
				try
				{
					...
					{
						log($"Failed to read {entry.FileName}");
						continue;
					}
				...
				{
					log($"Exception, failed to read {entry.FileName}, exception={ex}");
					continue;
				}
			}
	...
	public static async Task SetupSubObjects(ToolsSettings settings, Action<string> log)
	{
		var dir = settings.ObjectDirectory;
		var logger = new Logger();
		using var indexDb = LocoDbContext.GetDbFromFile(settings.DatabaseFile);
		...
		foreach (var objGroup in objects.GroupBy(x => x.ObjectType))
		{
			...
			foreach (var obj in objGroup)
			{
				...
				{
					log($"Couldn't find in index {firstDatObj.DatName} - {obj.Name}");
					continue;
				}

				if (entry?.FileName is null)
				{
					log($"Filename was null {firstDatObj.DatName} - {obj.Name}");
					continue;
				}
				...
				try
				{
					...
					if (o.LocoObject == null)
					{
						log($"Loco object was null {firstDatObj.DatName} - {obj.Name}");
						continue;
					}

					var result = await DbSubObjectHelper.AddOrUpdate(db, obj, o.LocoObject.Object);
					log($"{result} data for {firstDatObj.DatName} - {obj.Name}");
				}
				catch (Exception ex)
				{
					log(ex.ToString());
	...
	public static async Task QuerySubObjects(ToolsSettings settings, Action<string> log)
	{
		...
		foreach (var result in results)
		{
			log($"{result.Parent.DatObjects.First().DatName} has FirstSeason of {result.FirstSeason}");
		}

...
- [x] `ObjectService:DisableAuthentication` is now **forced off outside Development** (the dev scheme is only registered in Development, and an attempt to enable it elsewhere is reported on stderr), so production cannot disable authentication even if the setting leaks into configuration
...
- Full suite must stay green: `dotnet test Tests/Tests.csproj` (baseline: **2537 passed / 0 failed**, 5 skips).
...
| 2026-09-21 | Discovered issues | `DtoInfoResponse.UserName` (always null) removed and the Manage page now uses the signed-in principal; `PUT /v2/objects/{id}` now persists `SubObject` edits via the convention-based `SubObjectDtoMapper` (34 existing reverse mappers). Also forced `DisableAuthentication` off outside Development. +3 tests (2523 green) | _uncommitted_ |
| 2026-09-21 | Discovered issue 3 (string table) | `PUT /v2/objects/{id}` now replaces `StringTable` rows (PUT semantics: matched by name+language, omitted rows removed, empty clears); the Object Editor was fixed to send its string table instead of an empty one. No PATCH; clients send the whole object. +2 tests (2525 green) | _uncommitted_ |
| 2026-09-21 | PUT semantics sweep | Object update is now strict PUT throughout: an omitted `SubObject` removes the stored row (`DbSubObjectHelper.GetSubObjectRowAsync`) and the Object Editor now sends its sub-object too. The DTO fields the server deliberately will not accept (Name/ObjectType/ObjectSource/VehicleType/DatObjects/projections) are documented in code and in `ObjectService/README.md` instead of being silently ignored. +1 test (2526 green) | _uncommitted_ |
| 2026-09-21 | Object-data structure | `Definitions/Database/README.md` now documents the header-row (`Objects`) / sub-object (`Obj<ObjectType>`) / file-rows (`DatObjects`, one-to-many) structure, the `Obj<ObjectType>` naming convention and its single exception (`ObjectType.InterfaceSkin` → `ObjInterface`), and that `ObjectsMissing` is not an ignore list. Added `DbSubObjectStructureTests` (3 tests) locking the 1-1 `ObjectType` ↔ `Obj<ObjectType>` mapping in both directions, the `TblObject<ObjectType>` / `DbSubObject.Parent` shape, and that each type resolves to a real table the row lookup can query. `GetSubObjectRowAsync` now throws for an unmapped type like its sibling switches. +3 tests (2529 green) | _uncommitted_ |
| 2026-09-21 | PUT semantics (full row) | Per the structure note, `PUT /v2/objects/{id}` now applies **every changeable** `Objects` column plus the sub-object: the previous "server-derived, ignore the client" list is gone, and `DatObjects` stays immutable (it is the file on disk) while `ObjectSource` became server-owned in the next entry. New `ObjectUpdateResult`/`ObjectUpdateOutcome` give the route handler 400 (no name, or a sub-object of the wrong type), 403 (vanilla) and 409 (duplicate name, incl. the unique-index race) responses; `DbExceptionHelpers.IsUniqueConstraintViolation` turns the DB constraint into the same 409. Changing `ObjectType` removes the sub-object row of the previous type, and `DbSubObjectHelper` gained `GetTableSuffix`/`GetSubObjectTableType`/`IsSubObjectOfType` (convention-resolved, so the 34-arm switch stays only where it is needed). The Object Editor now round-trips `VehicleType` (`ObjectMetadata.VehicleType`) instead of sending null. +7 tests (2536 green) | _uncommitted_ |
| 2026-09-21 | Object source is server-owned | Follow-up on the full-row PUT: `ObjectSource` is not client-settable. Uploads already store `Custom` (and refuse vanilla files), while Steam/GoG/OpenLoco objects are placed in the server folders by hand, so a request asking for a different source is ignored and logged (`ObjectQueryService.UpdateAsync`). +1 test (2537 green) | _uncommitted_ |

## Discovered during remediation

Issues found while implementing that were not in the original review.

| Date | Area | Finding | Resolution |
| ------------ | ------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| 2026-09-20 | Frontend | `DtoInfoResponse.UserName` is never populated: ASP.NET Identity's `/manage/info` only returns `email` / `isEmailConfirmed`, so `Pages/Account/Manage` rendered an empty username. | ☑ **Fixed** — `UserName` removed from `DtoInfoResponse`; the page takes the display name from the signed-in principal (`User.Identity?.Name`). |
| 2026-09-21 | API | `PUT /v2/objects/{id}` accepted `DtoObjectPostResponse.SubObject` but silently ignored it, so sub-object edits (the Object Editor sends this on every edit) were discarded. | ☑ **Fixed** — `SubObjectDtoMapper` resolves the existing per-type `ToTbl...Entity` reverse mapper (34 of them) by convention; `ObjectQueryService.UpdateAsync` applies it via `DbSubObjectHelper.AddOrUpdate`, and (PUT semantics) an **omitted sub-object removes the row** via the new `DbSubObjectHelper.GetSubObjectRowAsync`. The Object Editor had `SubObject` commented out in `ObjectEditorContext`, so it now sends `cachedLocoObjDto.SubObject`. Guarded by `SubObjectDtoMapperTests` (throws instead of dropping data) plus PUT tests for apply/insert/remove. |
| 2026-09-21 | API | `PUT /v2/objects/{id}` also silently ignored `DtoObjectPostResponse.StringTable` — string-table edits via the API were discarded, the same class of bug as `SubObject`. | ☑ **Fixed** — `ObjectQueryService.SyncStringTable` replaces the rows (PUT semantics): rows are matched by name+language, added/updated, and rows the request omits are removed, so an empty table clears the string table. The Object Editor was sending an empty table with every metadata upload, so it was fixed to send `Model.LocoObject.StringTable.Table` instead. |
